...
import { AlertTriangle, ArrowLeft, Camera, Check, ImagePlus, Pencil, Plus, RefreshCw, X } from "lucide-react";
...
const n = (v, unit = "") => v == null ? "—" : `${Math.round(Number(v)).toLocaleString()}${unit}`;
...
// Sweeps a percentage from 0 → target on mount/update, driving the calorie ring's conic-gradient.
function useAnimatedPercent(target) {
  const [p, setP] = useState(0);
  useEffect(() => {
    if (reducedMotion()) { setP(target); return; }
    const raf = requestAnimationFrame(() => setP(target));
    return () => cancelAnimationFrame(raf);
  }, [target]);
  return p;
}

// Counts a hero figure up from 0 on mount instead of snapping straight to its value.
function CountUp({ value }) {
  ...
  return target == null ? "—" : Math.round(display).toLocaleString();
}
...
function MealRow({ meal, index = 0 }) { return <article className="meal-row" style={{ "--i": index }}><div className={`meal-image ${meal.color}`} aria-hidden="true"/><div className="meal-info"><strong>{meal.name}</strong><span>{meal.time}</span></div><b>{n(meal.calories)}</b></article>; }
function MealList({ meals }) { return meals.length ? <div className="meal-list">{meals.map((meal, i) => <MealRow key={meal.id} meal={meal} index={i}/>)}</div> : <p className="empty-state">No meals logged today.</p>; }
...
function HomeSkeleton() {
  return <section className="screen home-screen" aria-busy="true" aria-label="Loading your nutrition tracker">
    <header className="home-head"><div><i className="sk sk-eyebrow"/><i className="sk sk-title"/><i className="sk sk-pill"/></div><i className="sk sk-avatar"/></header>
    <i className="sk sk-ring-card"/>
    <div className="sk-grid-3">{[0, 1, 2].map(i => <i className="sk sk-macro" key={i}/>)}</div>
    <div className="sk-grid-3">{[0, 1, 2].map(i => <i className="sk sk-nutrient" key={i}/>)}</div>
    <i className="sk sk-row"/>
    <i className="sk sk-row"/>
  </section>;
}

function Onboarding({ step, setStep, profile, saveProfile, finish }) {
  const [goal, setGoal] = useState(goals[0][2]); const [activity, setActivity] = useState(activities[0][2]); const [birthdate, setBirthdate] = useState(""); const [sex, setSex] = useState("female"); const [heightCm, setHeightCm] = useState(""); const [weightKg, setWeightKg] = useState(""); const [savedProfile, setSavedProfile] = useState(null); const [saving, setSaving] = useState(false); const options = step === 0 ? goals : activities; const selected = step === 0 ? goal : activity; const targetProfile = savedProfile ?? profile;
  const next = async () => { if (step < 2) return setStep(step + 1); if (step === 3) return finish(); if (!birthdate || !heightCm || !weightKg) return; setSaving(true); try { setSavedProfile(await saveProfile({ goal, activityLevel: activity, birthdate, sex, heightCm: Number(heightCm), weightKg: Number(weightKg) })); setStep(3); } finally { setSaving(false); } };
  if (step === 3) return <section className="screen onboarding"><div className="steps"><i className="done"/><i className="done"/><i className="done"/><i className="done"/></div><p className="eyebrow">STEP 4 OF 4</p><h1>Your daily target is ready</h1><p className="subtitle">Your target is calculated and stored securely in your profile.</p><div className="target-card"><span>DAILY CALORIE TARGET</span><strong><CountUp value={targetProfile?.dailyCalorieTarget ?? targetProfile?.daily_calorie_target}/><em>kcal</em></strong></div><div className="target-macros"><Macro value={targetProfile?.proteinTargetG ?? targetProfile?.protein_target_g} label="Protein" tone="orange"/><Macro value={targetProfile?.carbsTargetG ?? targetProfile?.carbs_target_g} label="Carbs" tone="green"/><Macro value={targetProfile?.fatTargetG ?? targetProfile?.fat_target_g} label="Fat" tone="purple"/></div><button className="primary bottom-cta" onClick={next}>Start Tracking</button></section>;
  if (step === 2) return <section className="screen onboarding"><div className="steps"><i className="done"/><i className="done"/><i/><i/></div><p className="eyebrow">STEP 3 OF 4</p><h1>Tell us about you</h1><p className="subtitle">We use this to calculate a safe, personal daily target.</p><div className="profile-form"><label>Birthdate<input type="date" value={birthdate} onChange={e => setBirthdate(e.target.value)} required/></label><label>Sex<select value={sex} onChange={e => setSex(e.target.value)}><option value="female">Female</option><option value="male">Male</option><option value="other">Other</option></select></label><div><label>Height (cm)<input type="number" min="80" max="250" inputMode="decimal" value={heightCm} onChange={e => setHeightCm(e.target.value)} required/></label><label>Weight (kg)<input type="number" min="25" max="350" inputMode="decimal" value={weightKg} onChange={e => setWeightKg(e.target.value)} required/></label></div></div><button className="primary bottom-cta" disabled={saving || !birthdate || !heightCm || !weightKg} onClick={next}>{saving ? "Calculating…" : "See My Target"}</button></section>;
...
function App() {
  ...
  const boot = () => { setPage("loading"); setError(""); api.bootstrap().then(async result => { setProfile(result.profile); const complete = result.onboardingCompleted ?? result.onboarding_completed; if (complete) await loadData(); setPage(complete ? "home" : "onboarding"); }).catch(e => { setError(e.message); setPage("error"); }); };
  useEffect(() => { boot(); }, []);
  ...
  const saveProfile = async values => { const result = await api.completeOnboarding(values); setProfile(result.profile); await loadData(); return result.profile; };
  if (page === "loading") return <main className="app-shell"><HomeSkeleton/></main>;
  if (page === "error") return <main className="app-shell"><section className="screen status-screen"><AlertTriangle size={40}/><h1>Unable to load your data</h1><p>{error}</p><button className="primary retry-btn" onClick={boot}><RefreshCw size={18}/>Try again</button></section></main>;
